...
class _OrdersAdminScreenState extends State<OrdersAdminScreen> {
  ...
  void _refresh() =>
      setState(() => _future = _admin.fetchOrders(status: _statusFilter));
            ...
            builder: (context, snapshot) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, i) => _OrderRow(
                    order: orders[i], onTap: () => _openDetail(orders[i])),
              );
            },
          ),
        ),
...
class _OrderRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pill = AdminStatusPill(
        label: kOrderStatusLabels[order.status] ?? order.status,
          ...
          builder: (context, constraints) {
            ...
            return Row(
              children: [
                ...
                Expanded(
                    child: Text(Money.format(order.total),
                        style: const TextStyle(
                            fontSize: 13.5, color: AdminTheme.ink2))),
                Expanded(
                    child: Text(
                        '${order.createdAt.day}/${order.createdAt.month}/${order.createdAt.year}',
                        style: const TextStyle(
                            fontSize: 13, color: AdminTheme.muted))),
                pill,
                const SizedBox(width: 8),
            ...
            children: [
                  ...
                  ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      errorBuilder: (_, __, ___) => const Text(
                          'Image not found.',
                          style:
                              TextStyle(color: AdminTheme.muted, fontSize: 12)),
                    ),
                  ),
              ],
              const SizedBox(height: 16),
              ...
              DropdownButtonFormField<String>(
                ...
                items: [
                  for (final s in kOrderStatuses)
                    DropdownMenuItem(
                        value: s, child: Text(kOrderStatusLabels[s]!))
                ],
