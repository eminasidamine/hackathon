...
class _ProductsAdminScreenState extends State<ProductsAdminScreen> {
  ...
  void _refresh() {
    setState(() => _future = _admin.fetchAllProducts(
        shopId: _shopFilter,
  ...
  Future<void> _delete(Product p) async {
    final ok = await confirmDialog(context,
        title: 'Delete "${p.name}"?', message: 'This action cannot be undone.');
    if (!ok) return;
    try {
    ...
    return Column(
      ...
      children: [
        AdminPageHeader(
          title: 'Products',
          subtitle:
              "Le vendeur ET l'admin peuvent ajouter, modifier, supprimer",
        ...
        Padding(
          padding: EdgeInsets.fromLTRB(isNarrowAdmin(context) ? 16 : 28, 0,
              isNarrowAdmin(context) ? 16 : 28, 12),
          child: LayoutBuilder(
...
class _ProductRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusPill = AdminStatusPill(
        label: p.isVisible ? 'Visible' : 'Hidden',
        color: p.isVisible ? const Color(0xFF2E9C5B) : AdminTheme.muted);
    final salePill = p.isOnSale
        ? AdminStatusPill(
            label: 'Promo -${p.discountPercent}%', color: AdminTheme.red)
        : null;
    final actions = [
      Switch(
          value: p.isVisible,
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    Text('Stock: ${p.stock}',
                        style: const TextStyle(
                            fontSize: 12.5, color: AdminTheme.ink2)),
                    statusPill,
        ...
        return AdminCard(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                  child: Text(Money.format(p.price),
                      style: const TextStyle(
                          fontSize: 13.5,
                          fontWeight: FontWeight.w600,
                          color: AdminTheme.ink))),
...
class _ProductDialog extends StatefulWidget {
  ...
  const _ProductDialog(
      {required this.admin, required this.shops, this.existing});
...
class _ProductDialogState extends State<_ProductDialog> {
    ...
    try {
      ...
      if (_pickedImage != null) {
        ...
        final url = await _storage.uploadPublic(
            bucket: 'product-images', path: path, bytes: _pickedImage!.bytes);
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(
          widget.existing == null ? 'Nouveau produit' : 'Modifier le produit'),
      content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  items: widget.shops
                      .map((s) => DropdownMenuItem(
                          value: s.shop.id, child: Text(s.shop.name)))
                      .toList(),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText:
                          'Brand (optional — otherwise the shop name is shown)'),
                ),
                const SizedBox(height: 10),
                TextFormField(
                  controller: _name,
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    helperText:
                        "Leave empty = no promotion. Filled in and higher than the price: shows the crossed-out price + the discount.",
                    helperMaxLines: 2,
                  ),
                  validator: (v) {
