...
class _ShopsAdminScreenState extends State<ShopsAdminScreen> {
  ...
  Future<void> _delete(AdminShopRow row) async {
    final ok = await confirmDialog(
      ...
      message:
          'Its products and orders will be deleted too. This action cannot be undone.',
    );
    if (!ok) return;
    try {
              ...
              if (snapshot.hasError) {
                return Center(
                    child: Text('Error: ${snapshot.error}',
                        style: const TextStyle(color: AdminTheme.red)));
              }
  ...
  Widget build(BuildContext context) {
    final nameBlock = Column(
      ...
      children: [
        ...
        Text(row.ownerName ?? row.ownerEmail ?? row.shop.ownerId,
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    Text(row.shop.city ?? '—',
                        style: const TextStyle(
                            fontSize: 13, color: AdminTheme.ink2)),
                    Text(row.shop.whatsappPhone ?? '—',
                        style: const TextStyle(
                            fontSize: 13, color: AdminTheme.ink2)),
                    statusPill,
                  ],
        ...
        return AdminCard(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                  child: Text(row.shop.city ?? '—',
                      style: const TextStyle(
                          fontSize: 13, color: AdminTheme.ink2))),
              Expanded(
                  child: Text(row.shop.whatsappPhone ?? '—',
                      style: const TextStyle(
                          fontSize: 13, color: AdminTheme.ink2))),
              statusPill,
    ...
    if (!_formKey.currentState!.validate() || _selectedOwner == null) {
      if (_selectedOwner == null) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Choose an owner for the shop.')));
      }
      return;
    }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if (_selectedOwner != null)
                  Chip(
                    label: Text(_selectedOwner!.fullName ??
                        _selectedOwner!.email ??
                        _selectedOwner!.id),
                  ...
                  SizedBox(
                    ...
                    child: _results.isEmpty
                        ? const Center(
                            child: Text(
                                'Type a name or email to search for an account.',
                                style: TextStyle(
                                    color: AdminTheme.muted, fontSize: 12)))
                        : ListView.builder(
