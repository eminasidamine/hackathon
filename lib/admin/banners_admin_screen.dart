...
class _BannersAdminScreenState extends State<BannersAdminScreen> {
  ...
  Future<void> _addPhoto() async {
    ...
    try {
      ...
      final url = await _storage.uploadPublic(
          bucket: 'banner-images', path: path, bytes: bytes);
  ...
  Future<void> _delete(HomeBanner b) async {
    final ok = await confirmDialog(context,
        ...
        message: 'It will disappear from the banner rotation.');
    if (!ok) return;
    try {
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            ListTile(
              title:
                  const Text('Home', style: TextStyle(color: AdminTheme.ink)),
              trailing: _placementCategoryId == null
                  ? const Icon(Icons.check, color: AdminTheme.red)
                  : null,
              onTap: () =>
                  Navigator.of(context).pop(const _Placement(null, 'Accueil')),
            ),
            for (final c in categories)
              ListTile(
                title:
                    Text(c.name, style: const TextStyle(color: AdminTheme.ink)),
                trailing: _placementCategoryId == c.id
                    ? const Icon(Icons.check, color: AdminTheme.red)
                    : null,
                onTap: () =>
                    Navigator.of(context).pop(_Placement(c.id, c.name)),
              ),
            const SizedBox(height: 8),
          ],
        ),
    ...
    return Column(
      ...
      children: [
        AdminPageHeader(
          title: 'Banners',
          subtitle:
              'Up to ${AdminService.maxBanners} photos per location — they scroll automatically. '
          ...
          action: FutureBuilder<List<HomeBanner>>(
            ...
            builder: (context, snapshot) {
              ...
              return FilledButton.icon(
                ...
                label: Text(full
                    ? 'Maximum atteint ($count/${AdminService.maxBanners})'
                    : 'Ajouter une photo'),
              );
            },
          ),
        ...
        Padding(
          padding: EdgeInsets.symmetric(
              horizontal: isNarrowAdmin(context) ? 16 : 28),
          child: InkWell(
